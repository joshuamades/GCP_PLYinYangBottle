  ...
  async function processFolder(folder) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (SKIPPED_ASSETS.has(normalizedRelativePath)) {
          console.log(`Skipped unused asset: ${normalizedRelativePath}`);
          continue;
        }

        if (
          fileExt.toLowerCase() === 'png' &&
          fs.existsSync(path.join(parsedPath.dir, `${parsedPath.name}.webp`))
        ) {
          console.log(`Skipped PNG because WebP exists: ${normalizedRelativePath}`);
          continue;
        }

        const hasPngPair =
          fileExt.toLowerCase() === 'webp' &&
          fs.existsSync(path.join(parsedPath.dir, `${parsedPath.name}.png`));
        const moduleRelativePath = hasPngPair
          ? relativePath.replace(/\.webp$/i, '.png')
          : relativePath;
        const moduleExt = hasPngPair ? 'png' : fileExt;
        const fileNameWithoutExt = path.parse(moduleRelativePath).name;
        ...
        const exportName = `${camelCase(fileNameWithoutExt)}${moduleExt.toUpperCase()}`;
        const exportContent = `export const ${exportName} = "data:${mimeType};base64,${base64Content}";`;

        // Write to a .js file in the output folder
        const outputFilePath = path.join(outputFolder, moduleRelativePath.replace(/\\|\//g, '_') + '.js');
