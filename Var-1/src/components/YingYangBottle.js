...
const YIN_YANG_LEFT_DROPLET_COVERED_STAGE = 14;
...
const CENTER_BOTTLE_DEPTH = 3;
const CENTER_DROPLET_DEPTH = 4;
const CENTER_CAP_DEPTH = 5;
...
const YIN_YANG_CAP_LEFT_OFFSET_X = -0.075;
const YIN_YANG_CAP_RIGHT_OFFSET_X = 0.075;
const YIN_YANG_CAP_OFFSET_Y = -0.455;
const YIN_YANG_CAP_WIDTH = 0.18;
...
export class YingYangBottle {
  ...
  sealYinYangBottle(color) {
    ...
    cap.setAlpha(0);
    cap.setY(finalY - this.image.displayHeight * 0.12);
    cap.setScale(finalScaleX * 0.7, finalScaleY * 0.7);
    cap.setVisible(true);
  ...
  getDropletFillPercent(color, coveredStage) {
    const fillCount = this.getYinYangStage(color);
    const initialCount =
      color === "white"
        ? YIN_YANG_INITIAL_WHITE_STAGE
        : YIN_YANG_INITIAL_BLACK_STAGE;

    if (fillCount <= initialCount) {
      return 0;
    }

    const progressCount = fillCount - initialCount;
    const progressTarget = Math.max(coveredStage - initialCount, 1);
  ...
  getBlackCenterStreamPoints(startX, startY, direction) {
    const firstDrop = this.refCenterStreamDistance(200);
    const finalDrop = this.refCenterStreamDistance(200);
    const finalSideOffset = direction * this.refCenterStreamDistance(-60);
  ...
  getWhiteCenterStreamPoints(startX, startY, direction) {
    ...
    const finalSideOffset = direction * this.refCenterStreamDistance(-50);
